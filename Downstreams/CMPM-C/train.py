...
#***********************************************************************************************************************
    ...
    parser.add_argument('--image_path', type=str,
                        default=r'data/SYNTH_PEDES',
                        help='directory to store dataset')
    parser.add_argument('--dataset_path', type=str,
                        default=r'data/SYNTH_PEDES/synthpedes-dataset.json',
...
def train(args):
    ...
    # print(checkpoint['TxtEncoder_state_dict'])
    # print(model.text_encoder)
    # print()
    # for i in checkpoint["TxtEncoder_state_dict"]:
    #     print(i)
    checkpoint['TxtEncoder_state_dict'].pop("encoder.embeddings.position_ids")
    ...
    for epoch in range(epochs):
        ...
        optimizer.zero_grad()
        for idx, (images_gt, targets,  masks,  labels) in enumerate(train_dataloader):
            ...
            IDlabels = labels
            # IDlabels = IDlabels / max(IDlabels)
            # IDlabels.float()
            # IDlabels = IDlabels.type(torch.LongTensor)
            IDlabels = IDlabels.float()
            IDlabels = F.softmax(IDlabels, dim = 0)
            # print(IDlabels, "IDIDIDIIDIlabelsIDIDIDIIDIlabelsIDIDIDIIDIlabels")
            cmpm_loss, cmpc_loss, loss, image_precision, text_precision = Loss(global_visual_embed, global_textual_embed, IDlabels, device)
            #print(loss)
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
